...
class MatrixFormatter
  ...
  def format
    if arrs.length == 1
      lines = [
      separator,
      rows.collect { |row| [ format_row(row),sseparator] },
...
def sseparator
  ...
  def separator
    "+#{column_widths.map { |width| '-' * (max_len + 2) }.join("+")}+"
  end

  def lseparator
  	new_lines = []
  	lines = "+#{column_widths.map { |width| '-' * (max_len + 2) }.join("+")}+"
  	if isSquare
  ...
  def format_row(row)
    ...
    cells = 0.upto(row.length - 1).map do |ix|
        ...
        excess = max_len - row[ix].to_s.length
      if excess < 0
         

          row[ix].to_s
      elsif max_len == row[ix].to_s.length
          row[ix].to_s
      else
  ...
  def max_len
    ...
    columns.map { |column| column.max.to_s }.length
