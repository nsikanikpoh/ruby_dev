...
class PaginationHelper
  ...
  def page_item_count(page_index)
    ...
    ((page_index >= page_count) || (page_index < 0)) ? -1 : index_calc(indexes, page_index)

  end

  def index_calc(indexes, page_index)
    page_index < indexes.length-1 ? @items_per_page : @collection.length % @items_per_page
  ...
  def page_index(item_index) 
   ((item_index >= @collection.length) || (item_index < 0)) ? -1 : item_index_calc(item_index)
  end

   def item_index_calc(item_index)
      page = 0
      (0...@collection.length).to_a
      .each_slice(@items_per_page).to_a
      .each.with_index do |ar, index|
           page = index if ar.include?(item_index)
        end 
      page
  end
